package raitichan.raitisapi.math;
...
public class Bezier {
	...
	public static float bezierCurve (int start, int end, float t, int... p) {
		...
		if (length == 1) {
			throw new IndexOutOfBoundsException("ポイントがありません");
		}
		if (length == 2) {
	...
	public static float bezierCurve (float t, float... p) {
		int length = p.length - 1;
		if (length <= 1) {
			throw new IndexOutOfBoundsException("ポイント数が有効範囲外、ポイントがありません。ポイントは必ず3つ以上指定して下さい");
		}
		if (length == 2) {
